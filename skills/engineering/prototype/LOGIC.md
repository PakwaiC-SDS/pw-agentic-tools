...
Once the prototype has answered its question, capture the answer, then capture the prototype:

- Fold the validated logic (reducer / machine / function set) into the real module in `src/`.
- Save the HTML prototype file to `docs/prototype/<NNNN>-<feature-slug>/` as a primary source reference.
- Record the verdict (the question and what it settled) in the relevant issue or commit message.
