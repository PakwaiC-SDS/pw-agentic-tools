...
Once a variant has won, capture the answer (which variant and why), then clean up:

- Fold the winning variant into the real code in `src/`.
- Save the full set of variant files (including losers and the switcher) to `docs/prototype/<NNNN>-<feature-slug>/` as a primary source reference.
- Record the verdict in the relevant issue or commit message.

Cleanup by sub-shape:

- **Sub-shape A**: fold the winner into the existing page; drop the losing variants and the switcher from `src/`.
- **Sub-shape B**: promote the winning variant to a real route; drop the throwaway route and the switcher from `src/`.
